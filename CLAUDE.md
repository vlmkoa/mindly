...
| v10 | Anti-exemplar-leak rule: one WHAT TO AVOID line telling the model the opening example exchanges are per-user-identical calibration, not the user's words — production chats were occasionally referencing exemplar topics (democracy, Tokyo…) unprompted. **Not yet evaluated** — run the before/after eval pass (re-anchor the v9 baseline first per the 2026-07 harness note) before iterating further. |
